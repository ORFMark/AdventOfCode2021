...
public class GiantSquidPart2 implements Problem {
    ...
    public String run(String input) {
        ...
        for (int calledNumber : numbersToCall) {
            ...
            List<BingoBoard> boardsToRemove = new LinkedList<>();
            for(BingoBoard bingoBoard : boards) {
            ...
            for (BingoBoard boardToRemove : boardsToRemove) {
                if(boards.size() > 1) {
                    boards.remove(boardToRemove);
                }
            }
            if(boards.size() == 1 && boards.get(0).hasBingo()) {
                return Integer.toString(boards.get(0).calculateScore(calledNumber));
            }
        }
        return "";
    }
